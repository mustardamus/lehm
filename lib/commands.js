...
const templates = new Templates(config.templatesPath)
...
module.exports = class Commands {
  ...
  create (templateName) {
    const parseTemplate = function (templateName) {
      let templatePath = path.join(config.templatesPath, templateName)
      let filesArr = templatesObj[templateName].files
      let variables = parser.parseFiles(config.templatePath, filesArr)
